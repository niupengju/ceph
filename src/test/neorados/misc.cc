...
CORO_TEST_F(NeoRadosMisc, LongLocator, NeoRadosTest) {
  ...
  {
    auto p = pool();
    p.key(std::string(maxlen / 2, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
    auto p = pool();
    p.key(std::string(maxlen - 1, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
    auto p = pool();
    p.key(std::string(maxlen, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
...
CORO_TEST_F(NeoRadosMisc, LongNamespace, NeoRadosTest) {
  ...
  {
    auto p = pool();
    p.ns(std::string(maxlen / 2, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
    auto p = pool();
    p.ns(std::string(maxlen - 1, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
    auto p = pool();
    p.ns(std::string(maxlen, 'a'));
    co_await execute(oid,
                     WriteOp{}.write_full(bl), p);
  }
  {
